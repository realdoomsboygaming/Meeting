<template>
  <div class="all-reading">
    ...
    <div v-else-if="filteredContent.length === 0" class="empty-state">
      ...
      <h3>No content to continue reading</h3>
      <p>Start reading something to see your progress here!</p>
    ...
    <div v-else class="content-grid" :class="viewMode">
      ...
      >
        <div class="card-image">
          <img :src="item.poster || '/placeholder-book.jpg'" :alt="item.title" />
...
export default {
  ...
  setup() {
    ...
    const filteredContent = computed(() => {
      let content = [...libraryStore.continueReading]
    ...
    const formatLastRead = (date) => {
      ...
      const diffTime = Math.abs(now - read)
...
.continue-reading-card:hover .read-overlay {
  opacity: 1;
}
...
.content-grid.list .continue-reading-card {
  display: flex;
  align-items: center;
}
